...
$(function() {
	...
	//绑定回车事件下一个输入框聚焦
	$(".sportRecordArea").keydown(function() {
		var index = 0;
	...
	//运动时间变化时，对应的消耗卡洛里也变化
	$(document).on("change", ".inputH, .inputM", function() {
		var parTr = $(this).parent().parent().parent();
		var hour = Number(parTr.find(".inputH").val());
		var min = Number(parTr.find(".inputM").val());
		var calorie = Number(parTr.attr("calorie"));
		var timeLength = hour * 60 + min;
		parTr.find(".calories").text((timeLength / 30 * calorie).toFixed(1));
		//更新合计时间和总消耗能量
		showTotalByChange();
		//判断总时长是否超过24小时
		var isOver = isOverTime();
...
function showSportList(spoData) {
	$.post(basePath + "/sport/listSportsByName", spoData, function(ret) {
		if (ret.msg == 1) {
			...
			for (var int = 0; int < list.length; int++) {
				var obj = list[int];
				html += "<li><div class='daoruListFirst sport' sportId=" + obj.id + " calorie=" + obj.calorie + ">" + obj.name + "</div></li>";
			}
			$("#sportList").html(html);
		} else {
			layer.msg(ret.msgbox, {time : 1000});
...
function getRecordListHtml(daily) {
	...
	for (var int = 0; int < recordList.length; int++) {
		...
		listHtml += 
			"<tr sportId=" + obj.sportId + " calorie=" + obj.calorie + ">" +
				"<td>" + obj.sportName + "</td>" +
